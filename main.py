...
app = FastAPI(title="Photo Watermark Service")
...
def add_simple_watermark(image: Image.Image, text: str = "I support AMIT Maurya") -> Image.Image:
    """
    Add a simple circular watermark (easier implementation)
    """
    if image.mode != 'RGBA':
        image = image.convert('RGBA')
    ...
    circle_x = width - circle_size - 20
    circle_y = height - circle_size - 20
    
    # Draw circle
    draw.ellipse([circle_x, circle_y, circle_x + circle_size, circle_y + circle_size], 
                fill=(34, 139, 34, 200))  # Green with transparency
    
    # Add text
    try:
        font_size = max(12, circle_size // 10)
        font = ImageFont.truetype("arial.ttf", font_size)
    except:
        font = ImageFont.load_default()
    ...
    for word in words:
        ...
        if bbox[2] - bbox[0] < circle_size - 40:  # Leave margin
    ...
    # Draw text lines
    line_height = font.getbbox('A')[3] if hasattr(font, 'getbbox') else 20
